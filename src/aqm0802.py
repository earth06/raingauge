...
class AQM0802:
        ...
        for i in range(trials):
            try:
                self.bus.write_i2c_block_data(
                    self.address_aqm0802a,
                    self.register_setting,
                    [0x38, 0x39, 0x14, 0x70, 0x56, 0x6C],
                )
                sleep(0.2)
                self.bus.write_i2c_block_data(
                    self.address_aqm0802a, self.register_setting, [0x38, 0x0D, 0x01]
                )
                sleep(0.001)
                break
            except IOError:
    ...
    def write_char(self, c):
        ...
        self.bus.write_byte_data(
            self.address_aqm0802a, self.register_display, byte_data
        )
        self.position += 1
