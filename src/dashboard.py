...
raingauge = Raingauge()
raingauge.set_db()
df_daily = raingauge.get_precipitation(
    begin="2022-10-01 00:00:00", end="2022-10-31 23:59:59", freq="D"
)
del raingauge
            ...
            [
                html.Div(
                    [
                        dcc.Dropdown(
                            id="date-button",
                            options=[{"label": t, "value": t} for t in datetimes],
                            value="20221014",
                        )
                    ]
                ),
...
@app.callback(Output("graph", "figure"), Input("date-button", "value"))
def update_figure(date_button):
    ...
    df_daily = raingauge.get_precipitation(
        begin=s_daily_start, end=s_daily_end, freq="D"
    )
