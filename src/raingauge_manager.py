...
class Raingauge:
    ...
    def start_rain_observation(self):
        """observing rain"""
        state = self.pi.callback(
            self.SWITCH_PIN, pigpio.RISING_EDGE, self.record_rain_mass
        )
        try:
            while True:
                pass
    ...
    def get_precipitation(self, begin=None, end=None, freq="H"):
        ...
        sql = (
            f"SELECT * FROM raw_rain_data WHERE date_time BETWEEN '{begin}' AND '{end}'"
        )
        ...
        df_output = pd.merge(
            df_output, df_resample, how="left", left_index=True, right_index=True
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "mode", choices=["obs", "get", "clear", "clearall"], help=args_help
    )
